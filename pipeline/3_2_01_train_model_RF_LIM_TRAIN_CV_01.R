...
# get all files (if files are split)
pred_and_results_files <- list.files(preds_path_train_CV, full.names=TRUE)
preds_files <- pred_and_results_files[str_detect(pred_and_results_files,
                                                 preds_file)]

# preds eval
all_preds <- list()
for (preds_f in preds_files){
  load(preds_f)
